    ...
    * When `out` and `in` are stride-aligned, avoid indexing
    ...
    def _map(
        ...
        for i in prange(out.size):
            if out.size == in_storage.size and np.array_equal(out_shape, in_shape) and np.array_equal(out_strides, in_strides):
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    for p in prange(out.size):
        ...
        for i in range(reduce_dim_size):
            tmp += a_storage[initial_a_pos + i*offset_a] * b_storage[initial_b_pos + i*offset_b]
